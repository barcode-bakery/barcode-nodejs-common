...
class BCGFont {
    ...
    private box: TextMetrics | null = null;
     ...
     */
    constructor(private font: string, size: number | string) {
    ...
    draw(image: draw.Surface, x: number, y: number): void {
        let drawingPosition = this.getDrawingPosition(x, y);
        setFont(image, this.font, this.size);
        setColor(image, this.foregroundColor.allocate(image));

        image.save();
        image.translate(drawingPosition[0], drawingPosition[1]);
        let rotationAngle = this.getRotationAngle();
        image.rotate(-rotationAngle * Math.PI / 180);
        image.fillText(this.text, 0, 0);
        image.restore();
    }

    private getDrawingPosition(x: number, y: number): number[] {
    ...
    private getBox(): TextMetrics {
        if (!this.box) {
            let ctx = draw.imagecreatetruecolor(1, 1);

            setFont(ctx, this.font, this.size);
            this.box = ctx.measureText(this.text);
        }
